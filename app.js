...
btn.addEventListener("click", function (e) {
  e.preventDefault();
  const formValue = parseInt(form.value);
  if (isNaN(formValue) || formValue === 1 || formValue > 9 || formValue < 0) {
    randomNum = Math.floor(Math.random() * text.length);
    loremText.innerHTML = generateElement(randomNum);
  } else {
    tempText = text.slice(0, formValue);
    const final = tempText
      .map(function (item) {
        return `<p>${item}</p>`;
      })
      .join("");
    loremText.innerHTML = final;
  }
});
